    ...
    if st.session_state.attendance_images:
        ...
        for idx,img in enumerate(st.session_state.attendance_images):
            with gallery_cols[idx%4]:
                st.image(img,width="stretch",caption=f"{idx+1}")
    has_photos=bool(st.session_state.attendance_images)
    c1,c2,c3=st.columns(3)
    with c1:
        if st.button("Clear photos",width="stretch",type="tertiary",icon=":material/delete:"):
            st.session_state.attendance_images=[]
            st.rerun()
    with c2:
...
def teacher_tab_attendance_records():
    ...
    display_df=(summary.sort_values(by="ts_group",ascending=False)   ## Recent attendance comes at the top
                [["Time","Subject","Subject_Code","Attendance_Stats"]]) ## we are displaying time,subject,subject_code and attendance stats
...
def teacher_screen_login():
    ...
    teacher_pass=st.text_input("Enter your Password",placeholder="Enter Your Password")
    st.divider()
    btnc1,btnc2=st.columns(2)

    with btnc1:
        if st.button("Login",icon=":material/passkey:",type="secondary",width="stretch",shortcut="enter"):
...
def teacher_screen_register():
    ...
    teacher_password_confirm=st.text_input("Confirm your Password",type="password",placeholder="Enter Your Password")
    st.divider()
    btnc1,btnc2=st.columns(2)

    with btnc1:
        if st.button("Register Now",icon=":material/passkey:",type="secondary",width="stretch",shortcut="enter"):
            success,message=register_teacher(teacher_username,teacher_name,teacher_password,teacher_password_confirm)
            if success:
                st.success(message)
                import time
                time.sleep(2) ## display message for 2 seconds and then open login page
